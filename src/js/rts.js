    ...
    if (alert) {
      if (pga > variable.audio.pga) {
        if (pga > 200 && variable.audio.status.pga != 2) {
          if (UserCheckBox()["sound-effects-PGA2"] == 1) constant.AUDIO.PGA2.play();
          variable.audio.status.pga = 2;
        } else if (pga > 8 && !variable.audio.status.pga) {
          if (UserCheckBox()["sound-effects-PGA1"] == 1) constant.AUDIO.PGA1.play();
      ...
      if (i > variable.audio.shindo) {
        if (i > 3 && variable.audio.status.shindo != 3) {
          if (UserCheckBox()["sound-effects-Shindo2"] == 1) constant.AUDIO.SHINDO2.play();
          variable.audio.status.shindo = 3;
        } else if (i > 1 && variable.audio.status.shindo < 2) {
          if (UserCheckBox()["sound-effects-Shindo1"] == 1) constant.AUDIO.SHINDO1.play();
          variable.audio.status.shindo = 2;
        } else if (!variable.audio.status.shindo) {
          if (UserCheckBox()["sound-effects-Shindo0"] == 1) constant.AUDIO.SHINDO0.play();
