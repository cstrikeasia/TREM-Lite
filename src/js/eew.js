...
function show_eew(data) {
  ...
  if (!variable.eew_list[data.id] || variable.eew_list[data.id].cancel) {
    if (variable.eew_list[data.id] && variable.eew_list[data.id].cancel) {
      ...
      if (UserCheckBox()["sound-effects-EEW"] == 1) constant.AUDIO.EEW.play();
    ...
    if (data.serial > variable.eew_list[data.id].data.serial) {
      if (UserCheckBox()["sound-effects-Update"] == 1) constant.AUDIO.UPDATE.play();
  ...
  if (data.eq.max > 4 && !variable.eew_list[data.id].alert) {
    ...
    if (UserCheckBox()["sound-effects-EEW2"] == 1) constant.AUDIO.ALERT.play();
  }
